...
# Output sample of command "iwpriv wlan0 driver "ax_info get""
#Band0 Info:
#        Tx RU: 0
#        Tx BF report: 0
#        Rx BF HE BRP: 8
...
{
	AX_INFO=($(IWPRIV_DRV "ax_info get"))
	IFS=$'\t\n'
	...
	unset IFS
	IWPRIV_DRV "ax_info reset" > /dev/null
...
function log_wifi_kdm_metric
{
	case $1 in
		chip_active)
			...
			if [ $# -eq 5 ]; then
				log_to_kdm_minerva "operation=conn-soc-active;key=$2;value=$3;unit=ms;metadata=$4;metadata1=$5"
			else
		...
		host_wakeup)
			...
			if [ $# -eq 3 ]; then
				log_to_kdm_minerva "operation=wifi-num-wakeup-host;key=$2;value=$3"
			else
		...
		ant_switch)
			...
			if [ $# -eq 4 ]; then
				log_to_kdm_minerva "operation=AntSwitch;key=$2;value=$3;metadata=$4"
			elif [ $# -eq 5 ]; then
				log_to_kdm_minerva "operation=AntSwitch;key=$2;value=$3;metadata=$4;metadata1=$5"
			else
		...
		wifi_band)
			...
			if [ $# -eq 4 ]; then
				log_to_kdm_minerva "operation=Band;key=$2;metadata=$3;metadata1=$4"
			elif [ $# -eq 3 ]; then
				log_to_kdm_minerva "operation=Band;key=$2;metadata=$3"
			else
...
{
	log_to_kdm_minerva "operation=wifi-ax-macphy-info;key=Rx;value=${RXMURATIO};unit=s;metadata=${RXOFDMA_RATIO};metadata1=${RXRU_484ORMORE};metadata2=${RXRU_242ORLESS}"
}
...
function log_logcat
{
	...
	PHYMODE_WIFI6_HE=`echo $PHYMODE | grep ax`
	if [ -n "$PHYMODE_WIFI6_HE" ]; then
		log_80211ax_info
	fi
}
